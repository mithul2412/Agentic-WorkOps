# TicketOps: Real Office Workflow Simulation for End-to-End Work Item Tracking and Human-in-the-Loop Resolution
...
- Policy execution: `operate/`
- Integrations: `tools/`
- Persistent store: `storage/sqlite_store.py`
- UI: `ui/`
...
```bash
cd /Users/myth/Documents/VSCode/Codetor/ui
npm install
...
python scripts/generate_sample_tasks.py \
  --output-path /Users/myth/Documents/VSCode/Codetor/samples/tasks.json
```

### 2) Seed real Jira/Confluence/Bitbucket

```bash
python scripts/seed_real_systems.py \
  --project-key SCRUM \
  --count 24 \
  --confluence-drafts 6
```
...
```bash
python scripts/run_real_connected_demo.py \
  --base-url http://127.0.0.1:8000 \
  --project-key SCRUM \
  --seed-count 24 \
  --process-count 20 \
  --report-path /Users/myth/Documents/VSCode/Codetor/storage/real_connected_report.json
```

### 4) Run simulation batch

```bash
python scripts/run_poc_simulation.py \
  --base-url http://127.0.0.1:8000 \
  --tasks-path /Users/myth/Documents/VSCode/Codetor/samples/tasks.json \
  --max-tasks 40
```
